...
import boCustomersCreatePage from '@pages/BO/customers/create';
 ...
 * @extends BOBasePage
 */
class BOOrderCreatePage extends BOBasePage implements BOOrdersCreatePageInterface {
   ...
   */
  async getNoCustomerFoundError(page: Page): Promise<string> {
   ...
   * @returns {Promise<string>}
   */
  async getCustomerNameFromResult(page: Page, cardPosition: number = 1): Promise<string> {
   ...
   * @returns {Promise<string>}
   */
  async getCustomerCardBody(page: Page, cardPosition: number = 1): Promise<string> {
  ...
  async addProductToCart(
    ...
    // Add to cart — register response waiter BEFORE clicking to avoid the race condition
    // where the response arrives before the listener is registered.
    const addToCartResponse = page.waitForResponse(
      '**/sell/orders/carts/**/products**',
      {timeout: 30000},
    );
    await page.locator(this.addtoCartButton).click();
   ...
   * quantityMin: number}>}
   */
  async getProductDetailsFromTable(page: Page, row: number = 1): Promise<{
    reference: string,
    image: string,
    quantityMax: number,
    price: number,
   ...
   * basePrice: string}>}
   */
  async getProductGiftDetailsFromTable(page: Page, row: number = 1): Promise<{
    reference: string,
    image: string,
    quantity: number,
    price: string,
   ...
   * @returns {Promise<boolean>}
   */
  async isProductTableRowNotVisible(page: Page, row: number): Promise<boolean> {
   ...
   * @returns {Promise<boolean>}
   */
  async isVouchersTableNotVisible(page: Page): Promise<boolean> {
   ...
   * @returns {Promise<boolean>}
   */
  async isShippingBlockVisible(page: Page): Promise<boolean> {
   ...
   */
  async getSummaryDetails(page: Page): Promise<{
