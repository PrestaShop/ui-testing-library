...
class BOCarriersCreatePage extends BOBasePage implements BOCarriersCreatePageInterface {
  ...
  constructor() {
    ...
    this.addHandlingCostsToggle = (toggle: string) => `${this.carrierForm} #carrier_shipping_settings_has_additional_handling_fee_${toggle}`;
    ...
    this.costsPriceInput = (zoneId: number, from: number, to: number) => `${this.costsZoneBlock(zoneId)} input.form-control[data-from="${from}"][data-to="${to}"]`;
  ...
  async createEditCarrier(page: Page, carrierData: FakerCarrier): Promise<string> {
    ...
    await this.setChecked(page, this.enableToggle('1'), carrierData.enable);
    ...
    /// Zones - START
    // Remove all zones
    const locatorRangeZoneCheckboxes = await page.locator(this.zonesRemoveButton).all();
    for (const locatorRangeZoneCheckbox of locatorRangeZoneCheckboxes) {
      await locatorRangeZoneCheckbox.click();
    }
    // Add zones
    ...
    await this.setChecked(page, this.freeShippingToggle('1'), carrierData.freeShipping);
    if (!carrierData.freeShipping) {
      await this.setChecked(page, this.addHandlingCostsToggle('1'), carrierData.handlingCosts);
      ...
      // Remove all ranges
      const locatorRangesRemoveCheckboxes = await page.locator(this.rangesRemoveButton).all();
      for (const locatorRangesRemoveCheckbox of locatorRangesRemoveCheckboxes) {
        await locatorRangesRemoveCheckbox.click();
      }
      // Add ranges
  ...
  async setHandlingCosts(page: Page, toEnable: boolean = true): Promise<string> {
    await page.locator(this.tabShippingSettings).click();
    await this.setChecked(page, this.addHandlingCostsToggle('1'), toEnable);
