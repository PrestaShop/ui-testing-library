// @ts-ignore
import type {BOCurrenciesCreatePageInterface} from '@pages/BO/international/localization/currencies/create';
 ...
 */
class BOCreateCurrencyPage extends BOLocalizationBasePage implements BOCurrenciesCreatePageInterface {
  ...
  /**
     ...
     * @returns {Promise<string>}
     */
  async editCurrency(page: Page, currencyData: FakerCurrency): Promise<string> {
    await this.setValue(page, this.currencyNameInput(1), currencyData.name);
    await this.setValue(page, this.exchangeRateInput, currencyData.exchangeRate.toString());
    await this.setChecked(page, this.statusToggleInput(currencyData.enabled ? 1 : 0));
...
module.exports = new BOCreateCurrencyPage();
