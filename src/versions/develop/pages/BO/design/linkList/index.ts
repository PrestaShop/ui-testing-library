...
import type {BODesignLinkListPageInterface} from '@interfaces/BO/design/linkList';
 ...
 * @extends BOBasePage
 */
class LinkListPage extends BOBasePage implements BODesignLinkListPageInterface {
  ...
  constructor() {
    ...
    this.editRowLink = (hookName: string, row: number) => `${this.actionsColumn(hookName, row)} a[href*='edit']`;
   ...
   * @param row
   */
  async goToEditBlock(page: Page, hookName: string, row: number): Promise<void> {
    await this.clickAndWaitForURL(page, this.editRowLink(hookName, row));
  }
...
module.exports = new LinkListPage();
