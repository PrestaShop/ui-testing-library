...
import {
  BOMerchandiseReturnsEditPage as BOMerchandiseReturnsEditPageVersion,
} from '@versions/develop/pages/BO/customerService/merchandiseReturns/edit';

/**
 * Edit merchandise returns page for PrestaShop 9.1 (legacy AdminReturnController), reassigns the
 * legacy selectors and behaviour on top of the Symfony-migrated develop page object.
 * @class
 * @extends BOMerchandiseReturnsEditPageVersion
 */
class BOMerchandiseReturnsEditPage extends BOMerchandiseReturnsEditPageVersion implements BOMerchandiseReturnsEditPageInterface {
  protected status: string;

  protected productsTableDeleteColumn: (row: number) => string;

  protected continueButton: string;

  protected orderReturnSaveButton: string;

  protected orderReturnCancelButton: string;

  protected legacyFileName: string;

  /**
   * @constructs
   * Setting up titles and selectors to use on the legacy edit merchandise return page
   */
  constructor() {
    super();
    ...
    this.legacyFileName = '#fieldset_0 div.form-wrapper div:nth-child(8) div p:nth-child(1)';
  ...
  */
  ...
  async getFileName(page: Page): Promise<string> {
    return this.getTextContent(page, this.legacyFileName);
  }
  ...
  async downloadPDF(page: Page): Promise<string | null> {
    return this.clickAndWaitForDownload(page, `${this.legacyFileName} a`);
  }
