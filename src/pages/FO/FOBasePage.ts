...
export default class FOBasePage extends CommonPage implements FOBasePagePageInterface {
  ...
  constructor(theme: string = 'classic') {
    ...
    this.accountLink = '.user-info a[href*="my-account"]';
    this.h2AccountLink = '.ps-customersignin a[href*="my-account"]';
    this.logoutLink = '.user-info a[href*="?mylogout="]';
    ...
    this.autocompleteSearchResult = '.ui-autocomplete, .js-search-dropdown, #ps_searchbar_dropdown';
    this.autocompleteSearchResultItem = '.ui-autocomplete li.ui-menu-item, .js-search-dropdown li.search-result, '
      + '#ps_searchbar_dropdown a.ps-searchbar__result-link';
    this.autocompleteSearchResultItemLink = (nthChild) => `.ui-autocomplete li.ui-menu-item:nth-child(${nthChild}) a,`
      ...
      + `#ps_searchbar_dropdown a.ps-searchbar__result-link:nth-child(${nthChild})`;
  ...
  async changeLanguage(page: Page, lang: string = 'en'): Promise<void> {
    if (this.theme === 'hummingbird') {
      ...
      await this.selectByVisibleText(page, selLanguage, textContent!);
      return;
    }
    await Promise.all([
  ...
  async changeCurrency(page: Page, isoCode: string = 'EUR', symbol: string = '€'): Promise<void> {
    ...
    if (this.theme === 'hummingbird') {
      if (await page.locator(this.h2CurrencySelector).count() > 0) {
        await this.selectByVisibleText(page, this.h2CurrencySelector, currency);
        return;
      }
